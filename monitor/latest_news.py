...
# Mongo数据库配置和连接
try:
    with MongoClient('10.99.4.25', 27017) as client:
        db = client['z3dbus']
        db.authenticate('z3dbusread', 'z3dbusread')
        print('连接mongo生产数据库成功!')
...
if time_web > time_mongo:
    params = {'action': 'send_msg', 'users': 'chuanyun.feng|tao.liu', 'content':
              '智胜生产环境7x24小时新闻监控：' + '\n' +
              'mongo库Z3_NEWS表查询出来的7x24小时最新新闻与金融界网站获取到的不一致！' + '\n' +
              '从数据库查询到的最新新闻为：' + title_mongo + '，时间为：' + time_mongo + '\n' +
              '金融界网站获取的最新新闻为：' + title_web + '，时间为：' + time_web}
    requests.get('http://wx.jrj.com.cn/api/weixin.jsp', params=params)
