...
# 6.输入一个三位数，然后输出每个位置的数字（比如输入719，显示：百位数字：7，十位数字：1，个位数字：9）
# num = input('请输入一个三位数：')
# if num.isdigit():
#     if len(num) == 3:
#         num = int(num)
...
# 11.输入张三的语文、数学成绩，输出以下判断是否正确，正确（True）,错误（False）
# 张三的语文和数学都大于90分
...
# 20.输入一个大于1的值，然后算出从1加到这个数之和，比如输入6，求1+2+3+4+5+6
...
# 21.输入一个正数，求出从1开始到这个数中所有包含3数字的数和3的倍数的和
...
# 22.输入班级人数，然后依次输入所有学员的成绩，计算该班级学员的平均成绩和总成绩
...
# 25.输入一句话，判断大小多少个，小写多少，数字多少，其他多少
# s = input('输入一句话：')
# upper = []
# lower = []
# digit = []
...
# 28.水仙花数（三位数），比如 153 = 1的三次方 + 5的三次方 + 3的三次方
# for i in range(100, 1000):
#     if i == int(str(i)[0]) ** 3 + int(str(i)[1]) ** 3 + int(str(i)[2]) ** 3:
#         print('i=', i)

'''---------------------------------------------------------------------------有难度---------------------------------------------
# 29.100元买2元的铅笔，5元的铅笔盒，10元的文件夹，15元的彩笔，刚好花完，每样物品至少有一种，一共有多少种可能？打印出每一种组合
times = 0
for i in range(1, 51):
    for j in range(1, 21):
        for k in range(1, 11):
